...
public class MainViewModel extends AbstractViewModel {
    ...
    private final EntryListChangedListener mTabEntryListChangedListener;

    @Inject
    public MainViewModel(AppManager appManager, Navigator navigator, JobRepository jobRepository,
        ...
        mTabEntryListChangedListener = new EntryListChangedListener(this);
        rebindListener();
    }

    private void rebindListener() {
        mAdapterViewModel.clearAll();
        ObservableArrayList<TabEntryDto> entries = jobRepository.getEntries("dashboard");
    ...
    private class EntryListChangedListener extends ArrayListChangedListener<TabEntryDto> {

        private MainViewModel reference;
        final CompositeDisposable disposables = new CompositeDisposable();

        EntryListChangedListener(MainViewModel mainViewModel) {
            this.reference = mainViewModel;
        }
        ...
        public void onItemRangeInserted(ObservableArrayList<TabEntryDto> sender, int positionStart, int itemCount) {
            disposables.add(Single.fromCallable(() -> {
                for (int i = 0; i < itemCount; i++) {
                    ...
                    reference.mAdapterViewModel.addComponent(i, reference.getUi(entry), entry.getEntry());
                }
                return true;
                ...
                for (int i = 0; i < itemCount; i++) {
                    ...
                    if (entry.getListUi() != null) {
                        ...
                        if (reference.mAdapterViewModel.getComponents().size() > i + positionStart &&
                            ...
                            reference.mAdapterViewModel.updateComponent(i + positionStart, reference.getUi(entry));
                        }
                    }
                }
                return true;
    ...
    private class ListViewModelFeatures extends BasicConfigurableViewModelFeatures {
        ...
        public void openJob(String jobKey, UUID id) {
            navigator.navigateToJob(appManager.getContext(), id);
        }
        ...
        public void startJob(String jobKey) {
            ...
            navigator.navigateToJob(appManager.getContext(), msg.getId());
